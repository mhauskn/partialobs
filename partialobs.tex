...
: S \times A \rightarrow [0,1]$. At each timestep, the agent receives
...
A policy has an expected return: $\eta(\pi) = \Expect_{s_0, a_0,
  \dots} \Big [ \sum_{t=0}^\infty \gamma^t r(s_t) \Big ]$ where $a_t
\sim \pi(a_t|o_t)$. A reinforcement learning task induces at least one
optimal policy $\pi^*$, such that $\eta(\pi^*) \ge \eta(\pi)$ for all
$\pi$.

an optimal policy $\pi^*: \mathcal{O}
\rightarrow \mathcal{A}$ is a mapping from observation to action that
maximizes the expected returns $\mathcal{J} = \Expect_{\pi^*}[\sum_{t}
  \gamma^t r_t]$. The goal of reinforcement learning is to learn a
policy as close to optimal as possible. Building on the concept of
optimality, we define two properties of observation spaces:
